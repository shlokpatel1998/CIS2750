...
int main(int argc, char **argv) {
    ...
    if(newDoc != NULL) {
      char * t = wpt_list_json(newDoc->routes);
      printf("%s\n",t);
      free(t);
      deleteGPXdoc(newDoc);
    }

    char * t = gpx_to_html(argv[1]);
//    printf("%s\n", t);
...
//    printf("%s\n", t);
  ...
  //      validateGPXDoc(newDoc, argv[2]);
        /*
        GPXdoc * final = createValidGPXdoc("first.gpx", argv[2]);
        if(final != NULL) {
          bool valid = validateGPXDoc(final, argv[2]);
          deleteGPXdoc(final);
        }
        ...
        char * rts = routeListToJSON(NULL);
        printf("%s\n", rts);
        free(rts);

        char * ts = trackListToJSON(NULL);
        printf("%s\n", ts);
        free(ts);

        char * f = GPXtoJSON(NULL);
        printf("%s\n", f);
        free(f);
        */
