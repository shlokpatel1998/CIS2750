...
char * trackToJSON(const Track *tr) {
    if(tr != NULL) {
        ...
        ListIterator segs = createIterator(tr->segments);
        TrackSegment * ts = NULL;
        int total = 0;

        while((ts = nextElement(&segs)) != NULL) {
            total += getLength(ts->waypoints);
        }
...
char * wpt_to_json(Waypoint * wpt) {
    ...
    char * temp = createEmptyString();
    char * name = createEmptyString();
    char * lat = createEmptyString();
    char * lon = createEmptyString();


    if(checkStringEmpty(wpt->name)) catString("None", &name);
        
    else catString(wpt->name, &name);

    char d[50];
    sprintf(d, "%f", wpt->latitude);
    catString(d, &lat);

    char d2[50];
    sprintf(d2, "%f", wpt->longitude);
    catString(d2, &lon);
...
char * wpt_list_json(const List * list) {
     if(list != NULL) {
        char * temp = createEmptyString();
        int num_wpts = getLength((List*)(list));
        ...
        if( (num_wpts >= 1) ) {
            ...
            while((wpt = nextElement(&wpts)) != NULL) {
                if( (i == (num_wpts - 1)) ) {
                    char * t = wpt_to_json(wpt);
                    catString(t, &temp);
                    free(t);
                }
                else{
...
GPXdoc* JSONtoGPX(const char* gpxString) {
    ...
    char * namespace = createEmptyString();
    catString("http://www.topografix.com/GPX/1/1", &namespace);

    char * string = (char*)gpxString;
    ...
    double d = strtod(version, NULL);

    gdoc->version = d;

    copyNewString(name, &(gdoc->creator));

    strcpy(gdoc->namespace, namespace);
...
Waypoint* JSONtoWaypoint(const char* gpxString) {
    ...
    double longitude = strtod(lon, NULL);
...
bool create_gpx_file(char * filename, char * creator) {
    //version default is 1.1

    GPXdoc * doc = newGPXdoc();

    doc->version = 1.1;

    copyNewString(creator, &(doc->creator));
